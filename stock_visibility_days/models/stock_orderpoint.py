...
class StockWarehouseOrderpoint(models.Model):
    ...
    # region Core fields from 'stock' module
    visibility_days = fields.Float(
        compute='_compute_visibility_days',
        inverse='_set_visibility_days',
        string="Visibility Days",
        readonly=False,
        store=True,
        help="Consider the forecasted stock for this many days in the future"
             " when replenishing. Set to 0 for just-in-time."
    )
    ...
    # region Fields added by 'purchase_stock'
    ...
    # region Compute and Inverse Methods
    @api.depends('route_id', 'purchase_visibility_days', 'manufacturing_visibility_days')
    ...
    @api.depends('visibility_days', 'qty_multiple', 'qty_forecast',
        ...
        for orderpoint in self:
            ...
            if float_compare(
                ...
                remainder = orderpoint.qty_multiple > 0 and \
                    qty_to_order % orderpoint.qty_multiple or 0.0
    ...
    # region Procurement Trigger Logic (Aligned with V14)
    def _procure_orderpoint_confirm(
        ...
        for orderpoints_batch in split_every(1000, orderpoints_noprefetch):
            ...
            try:
                orderpoints_batch_records = self.env['stock.warehouse.orderpoint'].browse(
                    orderpoints_batch)
                orderpoints_exceptions = []
                while orderpoints_batch_records:
                    procurements = self._get_procurements(orderpoints_batch_records)
                    try:
                        ...
                        if hasattr(orderpoints_batch_records, '_post_process_scheduler'):
                            orderpoints_batch_records._post_process_scheduler()
                        break

                self._log_procurement_exceptions(orderpoints_exceptions)
            finally:
                if use_new_cursor:
                    try:
                        cr.commit()
    ...
    def _get_procurements(self, orderpoints):
        procurements = []
        for orderpoint in orderpoints:
            ...
            if float_compare(
                ...
                procurements.append(self.env['procurement.group'].Procurement(
                    ...
                    orderpoint.name, origin, orderpoint.company_id, values))
        return procurements

    def _log_procurement_exceptions(self, orderpoints_exceptions):
