...
class TestProductCategoryRestrictions(TransactionCase):
    ...
    def test_update_product_category_with_stock_move(self):
        ...
        with self.assertRaises(UserError):
            categ_all.write({"property_stock_account_input_categ_id": False})

        # for property_stock_account_output_categ_id
        with self.assertRaises(UserError):
            categ_all.write({"property_stock_account_output_categ_id": False})

        # for property_stock_valuation_account_id
        with self.assertRaises(UserError):
            categ_all.write({"property_stock_valuation_account_id": False})

        # for property_stock_journal
        with self.assertRaises(UserError):
            categ_all.write({"property_stock_journal": False})
